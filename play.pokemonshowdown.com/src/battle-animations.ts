	...
	getSidebarHTML(side: Side, posStr: string): string {
		...
		for (let i = 0; i < sidebarIcons.length; i++) {
			...
			if (iconType === 'empty') {
				...
				pokemonhtml += `<span${tooltipCode} style="${this.battle.dex.getPokemonIcon(poke, !side.isFar)};opacity:0.6" aria-label="${details}"></span>`;
			} else {
				const details = this.getDetailsText(poke);
				pokemonhtml += `<span${tooltipCode} style="${this.battle.dex.getPokemonIcon(poke, !side.isFar)}" aria-label="${details}"></span>`;
			}
